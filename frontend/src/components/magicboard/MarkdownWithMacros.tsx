...
function WorkItemMacroCard({ item }: { item: PageWorkItemSummary }) {
  return (
    <Link to={`/work-items/${item.id}`} className="macro-workitem-card">
...
function WorkItemMacro({ workspaceId, workItemKey }: { workspaceId: string; workItemKey: string }) {
  ...
  if (failed) {
    return <div className="macro-workitem-card macro-workitem-missing">Work item {workItemKey} not found</div>;
  }
  if (!item) {
