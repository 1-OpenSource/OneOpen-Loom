...
export default function MagicboardPageEditor() {
  ...
  const [linkedItems, setLinkedItems] = useState<PageWorkItemSummary[]>([]);
  ...
  const { data: projectsPage } = useApi(
    () =>
      workspaceId
        ? projectService.listProjects(workspaceId, { page_size: 100 })
        : Promise.resolve({ items: [], meta: { page: 1, page_size: 100, total: 0 } }),
    [workspaceId]
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      setIsLoading(true);
      const [pageResult, versionList, commentList, workItems, attachmentList, shareList] = await Promise.all([
        magicboardService.getPage(pageId),
        magicboardService.listPageVersions(pageId),
        magicboardService.listPageComments(pageId),
        magicboardService.listWorkItemsForPage(pageId),
      ...
      setWorkItemTitle(pageResult?.title ?? "");
      setVersions(versionList);
      setComments(commentList);
      setLinkedItems(workItems);
  ...
  async function handleCreateWorkItem(event: FormEvent) {
    event.preventDefault();
    if (!projectId || !workItemTitle.trim()) return;
    try {
      const item = await workItemService.createWorkItem(projectId, {
        title: workItemTitle.trim(),
        description: `Created from Magicboard page ${space.key}/${page?.slug ?? ""}.\n\n${content.slice(0, 2000)}`
      });
      await magicboardService.linkPageToWorkItem(item.id, { page_id: pageId });
      setLinkedItems(await magicboardService.listWorkItemsForPage(pageId));
      setIsCreateItemOpen(false);
      pushToast("Work item created and linked", "success");
    ...
    <div className="magicboard-editor">
      ...
      <div className="content-grid two-column-grid">
        ...
        <Card>
          <div className="section-heading">
            <h2>
              <Link2 size={16} /> Linked work items
            </h2>
            <span className="section-count">{linkedItems.length}</span>
          </div>
          <div className="list-stack">
            {linkedItems.length ? (
              linkedItems.map((item) => (
                <Link key={item.id} to={`/work-items/${item.id}`} className="list-row macro-workitem-card">
                  <span className="item-key">{item.work_item_key}</span>
                  <strong>{item.title}</strong>
                </Link>
              ))
            ) : (
              <p className="muted-copy">No linked work items.</p>
            )}
          </div>
          <Button variant="secondary" onClick={() => setIsCreateItemOpen(true)}>
            Create work item from page
          </Button>
        </Card>
      </div>
      ...
      <Modal isOpen={isCreateItemOpen} title="Create work item from page" onClose={() => setIsCreateItemOpen(false)}>
        <form className="form-stack" onSubmit={handleCreateWorkItem}>
          <Select
            label="Project"
          ...
          <div className="modal-actions">
            <Button type="button" variant="secondary" onClick={() => setIsCreateItemOpen(false)}>
              Cancel
            </Button>
            <Button type="submit" disabled={!projectId || !workItemTitle.trim()}>
              Create & link
            </Button>
          </div>
        </form>
